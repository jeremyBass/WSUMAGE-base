...
magneto-install:
  cmd.run:
    ...
    - user: root
    - cwd: {{ web_root }}
    - unless: ! php -r "require 'app/Mage.php';$app = Mage::app('default');$installer = Mage::getSingleton('install/installer_console');$installer->init($app);" 2>/dev/null | grep -qi "already installed"
    - require:
      - service: mysqld-{{ env }}
