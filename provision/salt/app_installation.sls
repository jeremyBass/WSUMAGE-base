...
magneto-install:
  cmd.run:
    ...
    - unless: php -r 'require "app/Mage.php"; try{ $app = Mage::app("default"); $installer = Mage::getSingleton("install/installer_console");  $installer->init($app); } catch (Exception $e) { Mage::printException($e); }' 2>&1 | grep -qi  'already installed' 
    - user: root
    - cwd: {{ web_root }}
    - require:
      - git: magento
