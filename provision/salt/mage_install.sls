...
magneto-install:
  cmd.run:
    - name: 'php bin/magento setup:install --language={{ magento['locale'] }} --timezone={{ magento['timezone'] }} --currency={{ magento['default_currency'] }}  --db-host="{{ database['host'] }}" --db-name="{{ database['name'] }}" --db-user="{{ database['user'] }}" --db-password="{{ database['pass'] }}" {% if database['prefix'] is defined and database['prefix'] is not none %} --db-prefix="{{ database['prefix'] }}" {%- endif %} --base-url={{ magento['url'] }} --use-rewrites={{ magento['use_rewrites'] }} --use-secure={{ magento['use_secure'] }} {% if magento['secure_base_url'] is defined and magento['secure_base_url'] is not none %} --base-url-secure={{ magento['secure_base_url'] }} {%- endif %} --backend-frontname="{{ magento['backend_frontname'] }}" --use-secure-admin={{ magento['use_secure_admin'] }} --admin-firstname="{{ magento['admin_firstname'] }}" --admin-lastname="{{ magento['admin_lastname'] }}" --admin-email="{{ magento['admin_email'] }}" --admin-user="{{ magento['admin_username'] }}" --admin-password="{{ magento['admin_password'] }}" && echo "export MagentoInstalled_Fresh=True {% raw %}#salt-set REMOVE{% endraw %}" >> /etc/environment'
    - user: root
    - cwd: {{ web_root }}
    - require:
      - cmd: magento
