...
base-ext-{{ ext_key }}:
  cmd.run:
    - name: modgit add {% if ext_val['tag'] is defined and ext_val['tag'] is not none %} -t {{ ext_val['tag'] }} {%- endif %} {% if ext_val['branch'] is defined and ext_val['branch'] is not none %} -b {{ ext_val['branch'] }} {%- endif %} {{ ext_key }} https://github.com/{{ ext_val['repo_owner'] }}/{{ ext_val['name'] }}.git | ADDED{{ ext_key|replace("-","") }}=true
    - cwd: {{ web_root }}
    - user: root
...
install-base-ext-{{ ext_key }}:
  cmd.run:
    ...
    - cwd: {{ web_root }}
    - user: root
    - onlyif: '[ -n "${{ '{' }}ADDED{{ ext_key|replace("-","") }}{{ '}' }}" ]'
    - require:
      - git: magento
      - service: mysqld-{{ env }}
      - service: php-{{ env }}
